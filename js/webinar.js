webinar ={
    ...
    coursesSingleDiv:function(singleCourse,isExpired){
        ...
        html+='</div><div class="views-count"><i class="fa fa-eye"></i>'+singleCourse.view+'</div>'+((isExpired)?'<a href="https://www.e3melbusiness.com/subscriptions" class="button btn-block text-center subscriptions-button">اشترك الان</a>':'')+'</div></div>';
        return html;
    },
    ...
    singleWebinarPage:function(){
        ...
        if(webinarID){
            el.getSingle(webinarID,function(msg){
                ...
                if(msg.success){
                    if(course.link){
                        $("#courseIframe").attr('src','https:'+course.link+'?playsInline=true').removeClass('hidden');
                    }else{
